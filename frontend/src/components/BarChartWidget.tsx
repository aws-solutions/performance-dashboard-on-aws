...
const BarChartWidget = (props: Props) => {
    ...
    }, [chartRef, props.showMobilePreview, props.bars, props.data, props.stackedChart]);
    ...
    const yAxisType = useCallback(() => {
        ...
        if (props.columnsMetadata && bars.length) {
            columnMetadata = props.columnsMetadata.find((cm) => cm.columnName === bars[0]);
        }
        ...
        <div aria-label={props.title} tabIndex={-1} className={props.title ? "" : "padding-top-2"}>
            ...
            {data && data.length > 0 && (
                <div aria-hidden="true">
                    <ResponsiveContainer id={props.id} width="100%" height={dims.chartHeight}>
                        <BarChart
                            className="bar-chart"
                            data={props.data}
                            layout="vertical"
