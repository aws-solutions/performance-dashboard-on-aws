...
export const writeSnapshot = function (name: string, snapshot: DashboardSnapshot) {
    fs.writeFileSync(
...
export const readResource = function (name: string, file: string) {
    return fs.readFileSync(`${__dirname}/../../resources/${name}/files/${file}`, "utf8");
};

export const writeResource = async function (name: string, file: string, readStream: Readable) {
