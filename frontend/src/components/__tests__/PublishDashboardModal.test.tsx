...
test("should render the dialog when close", async () => {
    ...
    expect(await screen.getByRole("heading", { name: "Publish Dashboard" })).toBeInTheDocument();
});
...
test("renders a submit button", async () => {
    renderModal(true);
    expect(await screen.findByRole("button", { name: "Publish dashboard" })).toBeInTheDocument();
});
