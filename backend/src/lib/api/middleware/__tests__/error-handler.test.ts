...
const successfulRouteHandler = jest.fn(async () => await Promise.resolve());
const failedRouteHandler = jest.fn(async () => await Promise.reject(new Error()));
...
describe("withErrorHandler", () => {
    ...
    it("handles ItemNotFound error and returns a 400 error", async () => {
        console.error = jest.fn();
        const itemNotFoundRouteHandler = jest.fn(
            async () => await Promise.reject(new ItemNotFound()),
        );
