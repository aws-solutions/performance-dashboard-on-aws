...
test("submits form with the entered values", async () => {
    ...
    userEvent.clear(screen.getAllByLabelText("Data visualization second color (optional)")[0]);
    userEvent.type(
...
test("invokes cancel function when use clicks cancel", async () => {
    ...
    expect(history.push).toHaveBeenCalledWith("/admin/settings/brandingandstyling");
});
