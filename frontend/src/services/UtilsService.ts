 ...
 * dashboards.
 */
function groupByTopicArea(dashboards: Array<Dashboard | PublicDashboard>): Array<PublicTopicArea> {
 ...
 */
function calculateYAxisMargin(largestTick: number, significantDigitLabels: boolean): number {
    const pixelsByCharacter = significantDigitLabels ? 2 : 8;
    const tickLocaleString: string = largestTick.toLocaleString();
    const numberOfCommas: number = (tickLocaleString.match(/[,\.]/g)?.length || 0) + 3;
...
function getSortData(columnName: string | undefined, isDescending: boolean | undefined): string {
    if (!columnName) {
        return "";
    }
...
function computeChartWidgetWidthPercent(
    ...
    const actualWidth =
        (largestHeader + 1) * headersCount * pixelsByCharacter + marginLeft + marginRight;
