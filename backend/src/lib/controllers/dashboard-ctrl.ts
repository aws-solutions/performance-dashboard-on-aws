...
async function updateDashboard(req: Request, res: Response) {
    const user = req.user;
    const { id } = req.params;
    const { name, topicAreaId, displayTableOfContents, description, updatedAt, tableOfContents } =
        req.body;
    ...
    try {
        topicArea = await TopicAreaRepository.getInstance().getTopicAreaById(topicAreaId);
    } catch (error) {
...
async function publishPendingDashboard(req: Request, res: Response) {
    ...
    const updatedDashboard = await repo.publishPendingDashboard(id, updatedAt, user, releaseNotes);
...
async function createNewDraft(req: Request, res: Response) {
    ...
    const draft = DashboardFactory.createDraftFromDashboard(dashboard, user, version);
...
async function copyDashboard(req: Request, res: Response) {
    ...
    const newDashboard = DashboardFactory.createCopyFromDashboard(origDashboard, user);
