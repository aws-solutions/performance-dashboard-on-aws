...
test("renders title", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Edit favicon" })).toBeInTheDocument();
});

test("renders page description", async () => {
    const { getByText } = render(<EditFavicon />, { wrapper: MemoryRouter });
    expect(getByText("This favicon will appear in the browser tab.")).toBeInTheDocument();
});
