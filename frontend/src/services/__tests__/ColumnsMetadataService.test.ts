...
describe("ColumnsMetadataService", () => {
    const sampleColumnMetadata: any = {
        ...
        currencyTypes: new Map<string, CurrencyDataType>([["price", CurrencyDataType["Dollar $"]]]),
    };

    test("should return columns metadata", () => {
        const { hiddenColumns, dataTypes, numberTypes, currencyTypes } = sampleColumnMetadata;
        ...
        expect(metadataResults.filter((columnMetadata) => columnMetadata.hidden)).toHaveLength(1);
        expect(
        ...
        expect(
            metadataResults.filter(
                (columnMetadata) => columnMetadata.numberType === NumberDataType.Percentage,
            ),
        ).toHaveLength(1);
        expect(
            metadataResults.filter(
                (columnMetadata) => columnMetadata.numberType === NumberDataType.Currency,
            ),
        ).toHaveLength(1);
        expect(
            metadataResults.filter(
                (columnMetadata) => columnMetadata.currencyType === CurrencyDataType["Dollar $"],
            ),
        ).toHaveLength(1);
    });
    ...
    test("should parse the columns metadata", () => {
        ...
        const parsedColumnsMetadata = ColumnsMetadataService.parseColumnsMetadata(columnsMetadata);
