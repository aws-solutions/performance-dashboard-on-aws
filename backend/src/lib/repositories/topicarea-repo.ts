...
class TopicAreaRepository extends BaseRepository {
    ...
    public async getDashboardCount(topicAreaId: string) {
        const result = await this.dynamodb.query({
            ...
            ProjectionExpression: "#pk, #sk, #type, #parentDashboardId, #version, #topicAreaId",
        ...
        dashboards.forEach((dashboard) => uniqueParents.add(dashboard.parentDashboardId));
    ...
    public async renameTopicArea(topicAreaId: string, name: string, user: User) {
        ...
        const dashboards = await dashboardRepo.listDashboardsInTopicArea(topicAreaId);
