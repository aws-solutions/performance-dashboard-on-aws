 ...
 */
async function generateOrValidate(dashboard: Dashboard, friendlyURL?: string): Promise<string> {
    if (!friendlyURL) {
        ...
        if (!isValid(friendlyURL)) {
            throw new InvalidFriendlyURL(`The provided dashboard URL ${friendlyURL} is invalid`);
        }
    }

    if (!(await isFriendlyURLAvailable(dashboard, friendlyURL))) {
        throw new InvalidFriendlyURL(`The dashboard URL ${friendlyURL} is already being used`);
    }
...
async function isFriendlyURLAvailable(dashboard: Dashboard, friendlyURL: string) {
