...
import { Dataset, DatasetItem, DatasetList, DatasetContent, SourceType } from "../models/dataset";
...
class DatasetRepository extends BaseRepository {
    ...
    public async listDatasets(): Promise<DatasetList> {
        ...
        return result.Items.map((item) => DatasetFactory.fromItem(item as DatasetItem));
    }
    ...
    public async uploadDatasetContent(content: DatasetContent): Promise<string> {
        ...
        try {
            await this.s3Service.putObject(this.bucketName, jsonKey, JSON.stringify(content));
        } catch (err) {
    ...
    public async updateDataset(id: string, metadata: any, data: DatasetContent) {
        ...
        try {
            await this.s3Service.putObject(this.bucketName, jsonKey, JSON.stringify(data));
        } catch (err) {
    ...
    public async getWidgetCount(datasetId: string) {
        return (await WidgetRepository.getInstance().getAssociatedWidgets(datasetId)).length;
    }
